...
> codebase or release state. Use [PLANS.md](../PLANS.md) for active sequencing
> and [CHANGELOG.md](../CHANGELOG.md) for implemented release history.

## How To Read This File

This is a preserved research notebook, not a maintained product overview or
backlog. Its sections have three historical roles:

- **Local Source Material Reviewed** records the project inputs and source
  interpretation available at the snapshot date.
- **Recent External Work** records the literature and software landscape used
  to shape early decisions.
- **Proposed System Shape**, **Validation Requirements**, and the roadmap
  sketch preserve the design hypothesis that preceded the current
  implementation.

Current usage belongs in `docs/`, current claims in
[CORRECTNESS.md](../CORRECTNESS.md), current priorities in
[PLANS.md](../PLANS.md), and durable decisions in
[`adr/`](../adr/README.md). Add a new dated research record when a later review
substantially changes this synthesis instead of editing old statements to
sound current.
