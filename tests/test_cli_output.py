...
def test_wds_metadata_explanation_and_preview_tables(monkeypatch) -> None:
    ...
    assert len(tables) == 2
    assert cli_output.wds_suitability_label({"status": "possible_totals_only"}) == (
        "Possible totals only"
    )
    assert cli_output.wds_suitability_label({"status": "unclear"}) == "Unclear"
    assert cli_output.wds_suitability_label({"status": "custom"}) == "custom"
    assert cli_output.wds_suitability_label(None) == ""
...
def test_source_and_tree_helper_tables(monkeypatch) -> None:
    ...
    assert len(tables) == 4
    assert cli_output.first_model_design_move(
    ...
    assert cli_output.first_model_design_move({"block_strategy": "custom"}) == "custom"
...
def test_cli_output_formatting_helpers() -> None:
    cli_output.print_wds_dimension_preview_table([])
    cli_output.print_wds_dimension_preview_table("not-a-list")
    cli_output.print_issues_table("not-a-list", title="Issues")
    cli_output.print_next_steps_table("not-a-list")
    ...
    assert cli_output.first_report_issue({"issues": []}) is None
    assert cli_output.first_report_issue({"issues": ["bad"]}) is None
    ...
    assert cli_output.format_date_range("2020-01-01", "2020-01-01") == "2020-01-01"
    assert cli_output.format_date_range("2020-01-01", "2021-01-01") == (
