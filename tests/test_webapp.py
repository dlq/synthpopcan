...
def test_webapp_assets_include_index() -> None:
    ...
    assert (root / "run-api.mjs").is_file()
    assert (root / "run-events.mjs").is_file()
    assert (root / "run-list.mjs").is_file()
    ...
    assert not (root / "statcan.mjs").exists()
    assert not (root / "wds-normalize.mjs").exists()
    assert not (root / "wds-selection.mjs").exists()
    assert not (root / "zip.mjs").exists()
