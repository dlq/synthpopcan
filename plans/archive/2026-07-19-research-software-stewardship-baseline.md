...
## Completed `1.0.0` Baseline

The completed `1.0.0` stewardship baseline covered the accepted model-
licensing policy and archive correction, full CFF validation, a verified
Software Heritage capture, dated
FAIR4RS and software-management records, supported-platform and maintenance
claims, the tested bilingual 2021 case study, and the exact stable
CLI/API/schema inventory and compatibility policy owned by the main roadmap.

Focused community introduction may proceed when useful. A model mirror, JOSS
submission, broader outreach campaign, or new publication surface remains
demand- and maturity-gated.

## Stewardship Baseline

The items below record the completed baseline and its continuing maintenance
obligations. They are not a second release history.
